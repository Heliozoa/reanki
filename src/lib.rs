...
impl Template {
    ...
    pub fn new(id: i32, name: String, question_template: String, answer_template: String) -> Self {
        Self {
            id,
            name,
...
impl ModelType {
    fn to_anki_json_format(&self) -> i32 {
        match self {
            Self::Standard => 0,
            // Self::Cloze => 1,
        }
...
impl Card {
    fn write_to_db(
        ...
        let card_ord: i32 = card_ord.into();
...
impl Note {
    ...
    fn write_to_db(
        &self,
        deck: &Deck,
        templates: &HashMap<i32, (i32, Arc<Template>)>,
        conn: &mut SqliteConnection,
    ) -> Result<(), Error> {
        use schema::notes;
        ...
        let note_id = diesel::insert_into(notes::table)
            .values((
                ...
                notes::mod_.eq(timestamp_secs as i32),
                notes::usn.eq(0),
                notes::tags.eq(tags),
...
impl Deck {
    ...
    pub fn add_note(&mut self, note: Note) {
        ...
        let mut new_template_ord = template_map.len() as i32;
        ...
        serde_json::json!(
            {
                self.id.to_string(): {
                    ...
                    // we use the deck id there, so we use the deck id here
                    "conf": self.id.to_string(),
                    // custom study
                    "extendNew": 0,
                    // custom study
                    "extendRev": 0,
...
impl Col {
    fn write_to_db(deck: &Deck, conn: &mut SqliteConnection) -> Result<(), Error> {
        use schema::col;
        ...
        let models = deck
            ...
            .map(|(m, templates)| {
                (
                    m.id.to_string(),
                    m.to_anki_json(deck.id, templates.values().map(|(_ord, t)| t.as_ref())),
                )
            })
            .collect::<Map<_, _>>();
        ...
        diesel::insert_into(col::table)
            .values((
                // creation timestamp
                col::crt.eq(timestamp_secs as i32),
                // modification timestamp
                col::mod_.eq(timestamp_millis as i32),
                // schema modification time
                col::scm.eq(timestamp_millis as i32),
                ...
                // decks json
                col::decks.eq(deck.to_value().to_string()),
                // deck config json
                col::dconf.eq(dconf.to_anki_json().to_string()),
                // tags, unsure
                col::tags.eq(tags),
            ))
            .execute(conn)
